...
def first_beats_second?(first, second)
  (first == VALID_CHOICES[0] && (second == VALID_CHOICES[2] || second == VALID_CHOICES[3])) ||
  (first == VALID_CHOICES[1] && (second == VALID_CHOICES[0] || second == VALID_CHOICES[4])) ||
  (first == VALID_CHOICES[2] && (second == VALID_CHOICES[1] || second == VALID_CHOICES[3])) ||
  (first == VALID_CHOICES[3] && (second == VALID_CHOICES[4] || second == VALID_CHOICES[1])) ||
  (first == VALID_CHOICES[4] && (second == VALID_CHOICES[2] || second == VALID_CHOICES[0]))
end
...
while playing
  prompt("Choose your ruleset: 1) Standard, 2) Big Bang Theory, 3) That 70's Show")
  answer = gets.chomp
  ...
  if player_score > computer_score
    prompt("Winner! You won #{player_score} out of the #{ROUNDS} rounds. Congrats!")
  else
    prompt("Loser! You lost #{player_score} out of the #{ROUNDS} rounds. Sorry!")
  end
